...
To test this hypothesis, the proposed approach followed five key steps:  
...
### Seats vs. Actual Passengers [Check next!]
...
![Expand for Eurocontrol and important finding! Extremely promising for European-centric studies] ! ... + OpenSky in combination. ! or further below.
...
For a more detailed incubation-lag analysis, one could instead use an SEIR framework, as described by [@abou2020]. This model adds an Exposed (E) compartment for individuals who are infected but not yet infectious. This extra compartment accounts for incubation periods and can yield more realistic dynamics for diseases like COVID-19. However, it also adds complexity—more parameters to estimate and a lengthier calibration process. Given the scope of my bachelor thesis, the simpler SIR approach may suffice to derive approximate arrival times, but future work could adopt SEIR for a more detailed incubation-lag analysis.
...
# Refinement of Research Design and Methodology

[Not done yet, just a placeholder]

Based on this evaluation, my refined approach will prioritize

- European Centric GMN Construction
- SIR Model Implementation
- Weighted Nodes with more Traffic Data (Paris, London, Frankfurt, Amsterdam, Madrid, Rome, etc.)
- Excess Mortality Correlation for European Countries
- Use of Data: OpenSky, EUROCONTROL (! Extremely promising, further research ongoing, still need Acception of EUROCONTROL), certain Covid-19 datasets
- Refining the Research Design Section
- Finalizing the Methods (ODE details, parameter choices, simulation steps) - ODE LaTeX equations ..
- Clarifying the expected statistical approach – use correlation, regression, causal inference, etc.?




### Christian's Feedback (Summary & Clarification)

Christian emphasized the importance of **simplifying the modeling approach** today (14.03), to ensure feasibility and clarity for the bachelor thesis. He specifically advised **against using complex agent-based modeling (ABM)**, as explicitly modeling individual passenger movements—including return journeys, such as travelers flying from Frankfurt and subsequently returning home—would introduce substantial complexity beyond the thesis scope.

Instead, Christian recommended a more streamlined approach, employing a network-based SIR model similar to Brockmann & Helbing (2013), structured as follows:

- **Nodes represent airports or national-level aggregates**, serving as the core spatial units of analysis.
- **Edges reflect aggregated daily passenger flows** between nodes. These flows are calculated by multiplying **seat capacity by passenger load factors**, acting as simplified approximations of true passenger numbers.
- **Daily passenger volumes** become the primary drivers of infection intensity within each node, analogous to treating airports as interconnected nodes or junctions linked by pipelines. This way, higher passenger volumes translate into stronger transmission potential or larger "pipelines."
- Each airport’s daily infection rate is then determined by these incoming aggregated passenger flows rather than explicit tracking of individual passengers.

This approach effectively simplifies the model by transforming complex individual travel patterns into manageable aggregated flows, enhancing feasibility while preserving the critical aspects needed to test my hypothesis. Therefore, I maintain a clear, feasible model that accurately captures key epidemic spread dynamics without being overwhelmed by unmanageable computational complexity.

In summary:

- **Nodes:** Airports or country-level nodes.
- **Edges:** Daily aggregated passenger numbers (based on seat capacity × load factor).
- **Infection Dynamics:** Driven by incoming passenger volume per node, using differential equations for each day and node.

- Also, I need something to make it more pop at the beginning

- Nodes (airports) + edges (passenger flow) + infection dynamics over time.
- Simple graph visual showing high-traffic nodes (Paris, London, Frankfurt) vs. low-traffic nodes.
- Maybe an epidemic spread animation later in the results? (even if basic) .. or a static map *Sankey diagram showing passenger flows between major European hubs) ! -> Before diving into the SIR Model!
- https://www.sciencedirect.com/science/article/pii/S0277953620308741 next paper to read about Spatial contagion among local authority: "COVID-19 does not stop at open borders: Spatial contagion among local authority districts during England's first wave"
## Conclusion ? [Needs tone of work]

We hypothesize that early epidemic seeding systematically increased excess mortality by:
- Limiting early interventions—Governments had less time to implement non-pharmaceutical measures (NPIs).
- Prolonging exposure—Early-hit regions faced multiple waves before vaccines were available.
- Worsening disparities—Healthcare systems with fewer resources struggled more over extended outbreaks.

To test this, we:
- Smulate epidemic spread using an ODE-based SIR model on a Global Mobility Network (GMN).
- Estimate epidemic arrival ranks across European countries.
- Correlate early arrival with excess mortality & socioeconomic indicators.


## Subchapter Structure for Thesis

- Introduction (Introducing the problem, research questions, and why this matters.)
- Literature Review 
  - Establishing a scientific basis for the study, summarizing key findings, and identifying gaps. - Network Epidemiology and SIR Models (Classical SIR Models -> Network based -> Applications to flight networks and pandemics
  - COVID-19 & Global Mobility Networks (GMN) (Early studies on aviation-driven spread [@chinazzi2020; @sun2022_a] - Excess mortality: What drives national differences?)
  - Data Challenges in Flight-Based Epidemiology - ADS-B Data (Coverage & Bias) - Passenger vs. Seat-Capacity Issues - Ghost Flights & Data Uncertainty
- Methodology (Explain tools, data, models, and statistical methods used.)
  - Data Sources and Feasibility Challenges
  - Model Implementation (SIR or SEIR)
  - Simulation Steps (ODEs, parameter choices, etc.)
- Data Sources and Feasibility Challenges
- Result and Analysis
- Discussion - Does Early Arrival Matter for COVID-19 Outcomes? - How Do Data Biases Affect Model Reliability?
  - Limitations & Future Work
    - Missing Data & Passenger Uncertainty
    - Expanding beyond Europe
    - Alternative modeling approaches (SEIR, Agent-Based Models)

- Conclusion - Summarizing key takeaways and implications for future research. without repeating the Discussion.
- Ref (APA!)
